...
当前状态：本地评审稿，尚未推送 GitHub、尚未进行 K230/STM32/MSPM0G3507 三端实机验证。Python 与纯 C 参考实现已通过自动测试，C 核心与 STM32 HAL 适配层已通过 Keil ARMCC 编译检查；未经实机测试不得标记为正式稳定版。
...
正式发布前仍需补齐：K230、STM32、MSPM0G3507 的最终 UART 引脚表，并完成三块硬件的连续通信、断线保护和恢复测试。
