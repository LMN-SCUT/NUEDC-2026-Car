...
当前状态：v1 实机验证候选版。Python 与纯 C 参考实现已通过自动测试，C 核心与 STM32 HAL 适配层已通过 Keil ARMCC 编译检查。2026-07-22 已完成 K230 ↔ STM32F407 双向实机验证：K230 UART3（GPIO32/33）以 115200 8N1 发送观测帧和心跳，F407 UART4（PC10/PC11）能够持续接收并通过 CRC 解析；F407 周期发送 `SET_MODE`，K230 能正确解析、切换模式并返回 ACK。拔掉串口线后主控能够停止使用旧数据，重新接线后无需复位即可自动恢复通信。同日完成 K230 ↔ MSP-LITO-G3507 双向实机验证：MSPM0G3507 使用 UART1（PA8/PA9），能够持续接收并解析观测帧与心跳、发送 `SET_MODE`、关联成功 ACK 并确认 K230 模式切换；拔掉 PA9 接收线后 D1 停止随心跳闪烁，恢复接线后重新工作。STM32F407 与 MSPM0G3507 现均复用同一协议核心。
...
| 设备 | UART | TX | RX | 备注 |
|---|---|---|---|---|
| K230 亚博智能板 | UART3 | GPIO32 | GPIO33 | 两种主控共用 |
| STM32F407VET6 | UART4 | PC10 | PC11 | HAL 工程稳定基线 |
| MSP-LITO-G3507 | UART1 | PA8 | PA9 | 当前 TI 正式联调配置 |

MSP-LITO-G3507 的 UART0 PA10/PA11 同时连接 J3 的 XDS110 backchannel UART，完整连接调试器时可能与外部设备竞争，因此不作为 K230 正式通信口。
...
当前实现进度：STM32F407 与 MSPM0G3507 均已采用中断/环形缓冲接收，并实现 20 ms 半帧超时、150 ms 观测失效和 500 ms 链路失联判断。STM32 暴露 `vl_hal_observation_is_fresh()` 与 `vl_hal_link_is_alive()` 供控制循环进入安全状态；TI 端通过 `g_observationFresh`、`g_linkAlive` 公开状态，并在失联时清除 ACK、模式确认和旧观测状态。2026-07-22，MSPM0G3507 已通过噪声、坏 CRC、非法长度、截断帧、重复命令、拔线失联及重新接线自动恢复测试；截断帧准确触发一次解析器超时，测试结束后合法帧继续增长且接收环形缓冲无溢出。正式发布前仍需完成连续 10 分钟稳定性测试，以及 STM32F407 端同等的异常流实机复测。
