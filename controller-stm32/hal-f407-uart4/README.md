...
使用 K230 的 `vision-k230/examples/uart_protocol_smoke_standalone.py` 依次注入随机噪声、错误 CRC、非法长度头和截断帧，随后恢复正常观测、心跳及命令应答流。STM32F407 实测结果：

- 四类异常均不会形成有效观测，也不会卡死接收状态机。
- 截断帧停顿超过 20 ms 后能够丢弃半帧并重新寻找帧头。
- 正常数据恢复后无需复位，PB0/PB1 指示逻辑重新工作，双向命令与 ACK 继续正常。
- 拔掉串口连接后旧观测按 150 ms 失效、链路按 500 ms 判定失联；重新接线后能够自动恢复。
- Keil 工程重新编译结果为 `0 Error(s), 0 Warning(s)`，并已通过 USB ISP 实机烧录验证。

因此本工程与 MSPM0G3507 工程共用的协议 v1 核心可作为后续视觉算法和控制代码的稳定通信层。K230 脚本中的 `FAULT_INJECTION_ENABLED` 默认必须保持为 `False`，仅在专门复测异常恢复时临时打开。
